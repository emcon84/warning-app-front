...
export function StoreProductRow({ producto, slug, isDark, cardBg, textPri, textSec, textMuted, onToggle, onDelete, onEdit }: Props) {
  ...
  const actions = (
    <div className="flex flex-wrap items-center gap-1.5">
      ...
      <button
        ...
        title={shareCopied ? "Copiado!" : "Copiar link"}
  ...
  return (
    <div className={`rounded-2xl border overflow-hidden ${cardBg} flex flex-row md:flex-col`}>
      {/* Imagen — izquierda en mobile, arriba en desktop */}
      {producto.foto ? (
        <div className="relative w-24 flex-shrink-0 self-stretch md:w-full md:h-44 md:flex-shrink-0">
          <Image
        ...
        <div className={`w-24 flex-shrink-0 self-stretch md:w-full md:h-44 flex items-center justify-center ${isDark ? "bg-gray-800" : "bg-gray-100"}`}>
          <ImageOff className={`w-6 h-6 ${isDark ? "text-gray-600" : "text-gray-300"}`} />
        </div>
      )}
