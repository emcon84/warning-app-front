...
import { CheckCircle, Briefcase, FileText, User, Building2 } from "lucide-react";
...
const BARRIOS = [
  ...
  "Las Lomas", "Parque Industrial", "Barrio Newbery", "Los Lapachos",
  "San Cayetano", "Otro",
];
...
const HABILIDADES_SUGERIDAS = [
  ...
  "Gastronomía", "Informática", "Limpieza", "Logística",
  "Mantenimiento", "Marketing", "Mecánica", "Panadería",
  "Plomería", "Recepción", "Seguridad", "Ventas",
];
    ...
    return (
      <div className={`min-h-screen ${bg} pb-24 md:pb-0`}>
        ...
        <div className="flex flex-col items-center justify-center min-h-[60vh] pt-16 px-6 text-center gap-6">
          <div className={`w-16 h-16 rounded-full flex items-center justify-center ${isDark ? "bg-emerald-900/40 border border-emerald-700" : "bg-emerald-50 border border-emerald-200"}`}>
          ...
          <div>
            ...
            <p className={`text-sm ${textSec}`}>Necesitas una cuenta de comercio para publicar vacantes.</p>
          </div>
  ...
  if (!hasComercio) {
    return (
      <div className={`min-h-screen ${bg} pb-24 md:pb-0`}>
        <Navbar totalReports={0} onMenuClick={() => {}} sidebarDisabled mapView="profesionales" />
        <div className="flex flex-col items-center justify-center min-h-[60vh] pt-16 px-6 text-center gap-6">
          <div className={`w-16 h-16 rounded-full flex items-center justify-center ${isDark ? "bg-amber-900/40 border border-amber-700" : "bg-amber-50 border border-amber-200"}`}>
            <Building2 className={`w-8 h-8 ${isDark ? "text-amber-400" : "text-amber-600"}`} />
          </div>
          <div>
            <h1 className={`text-xl font-bold mb-2 ${isDark ? "text-white" : "text-gray-900"}`}>Necesitas un perfil de comercio</h1>
            <p className={`text-sm ${textSec} max-w-xs`}>Solo los comercios registrados pueden publicar vacantes. Primero crea tu perfil de comercio.</p>
          </div>
    ...
    return (
      <div className={`min-h-screen ${bg} pb-24 md:pb-0`}>
        ...
        <div className="flex flex-col items-center justify-center min-h-[60vh] pt-16 px-6 text-center gap-6">
          ...
          <div>
            <h1 className={`text-2xl font-bold mb-2 ${isDark ? "text-white" : "text-gray-900"}`}>Vacante publicada</h1>
            <p className={`text-sm ${textSec}`}>Los candidatos ya pueden ver y postularse a tu vacante.</p>
          </div>
          <div className="flex flex-col sm:flex-row gap-3">
            <button
  ...
  return (
    <div className={`min-h-screen ${bg} pb-24 md:pb-0`}>
      ...
      <div className="max-w-lg mx-auto px-4 py-8">
        ...
        {/* Header */}
        <div className="mb-6">
          <h1 className={`text-xl font-bold ${isDark ? "text-white" : "text-gray-900"}`}>Publicar vacante</h1>
          <p className={`text-sm mt-1 ${textSec}`}>Contale a los candidatos que estas buscando.</p>
        </div>
                ...
                <div className="flex flex-col items-center gap-1">
                  <div className={`w-9 h-9 rounded-full flex items-center justify-center border-2 transition-all ${
                    ...
                    {done
                      ...
                      : <Icon className={`w-4 h-4 ${active ? isDark ? "text-emerald-400" : "text-emerald-600" : isDark ? "text-gray-600" : "text-gray-400"}`} />
                    }
                  </div>
                  <span className={`text-[10px] font-medium hidden sm:block ${active ? isDark ? "text-emerald-400" : "text-emerald-600" : isDark ? "text-gray-600" : "text-gray-400"}`}>
                    {s.label}
                  </span>
                </div>
                {idx < steps.length - 1 && (
                  <div className={`flex-1 h-0.5 mx-2 ${step > s.num ? "bg-emerald-600" : isDark ? "bg-gray-800" : "bg-gray-200"}`} />
                )}
              </div>
            );
          })}
        ...
        <div className={`rounded-2xl border p-6 ${cardBg}`}>
          ...
          {step === 1 && (
            <div className="flex flex-col gap-5">
              <div>
                <h2 className={`text-lg font-bold mb-1 ${isDark ? "text-white" : "text-gray-900"}`}>La vacante</h2>
                <p className={`text-sm ${textSec}`}>Describile a los candidatos que estas buscando.</p>
              </div>

              <div className="flex flex-col gap-1.5">
                <label className={`text-sm font-medium ${labelClass}`}>Puesto / titulo *</label>
                <input
                  type="text"
                  value={titulo}
              ...
              <div className="flex flex-col gap-1.5">
                <label className={`text-sm font-medium ${labelClass}`}>Descripcion *</label>
                <textarea
                  value={descripcion}
                ...
                />
                <p className={`text-xs text-right ${textSec}`}>{descripcion.length}/1000</p>
              </div>
          ...
          {step === 2 && (
            <div className="flex flex-col gap-5">
              <div>
                <h2 className={`text-lg font-bold mb-1 ${isDark ? "text-white" : "text-gray-900"}`}>Detalles</h2>
                <p className={`text-sm ${textSec}`}>Cuanta mas info, mas facil encontrar al candidato ideal.</p>
              </div>

              {/* Habilidades */}
              <div className="flex flex-col gap-1.5">
                <label className={`text-sm font-medium ${labelClass}`}>Que buscamos (habilidades)</label>
                <div className={`flex flex-wrap gap-2 p-3 rounded-xl border min-h-[48px] ${isDark ? "bg-gray-800 border-gray-700" : "bg-white border-gray-300"}`}>
                  {habilidades.map((h) => (
                    <span key={h} className={`flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-medium ${isDark ? "bg-emerald-900/50 text-emerald-300 border border-emerald-700" : "bg-emerald-50 text-emerald-700 border border-emerald-200"}`}>
                      {h}
                      <button onClick={() => removeHabilidad(h)} className="ml-0.5 opacity-60 hover:opacity-100">
                  ...
                  <input
                    ...
                    placeholder={habilidades.length === 0 ? "Escribi una y Enter" : "Agregar mas..."}
              ...
              {/* Horario */}
              <div className="flex flex-col gap-1.5">
                <label className={`text-sm font-medium ${labelClass}`}>Horario (opcional)</label>
                <input
                  type="text"
                  value={horario}
              ...
              {/* Salario */}
              <div className="flex flex-col gap-1.5">
                <label className={`text-sm font-medium ${labelClass}`}>Remuneracion (opcional)</label>
                <input
                  type="text"
                  value={salario}
              ...
              {/* Modalidad */}
              <div className="flex flex-col gap-1.5">
                <label className={`text-sm font-medium ${labelClass}`}>Modalidad (opcional)</label>
                  ...
                  {MODALIDADES.map((m) => (
                    <button
                      ...
                      className={`px-3 py-1.5 rounded-full text-xs font-medium border transition-colors ${
                        modalidad === m
                          ? isDark ? "bg-emerald-900/50 text-emerald-300 border-emerald-700" : "bg-emerald-50 text-emerald-700 border-emerald-300"
                          : isDark ? "bg-gray-800 text-gray-400 border-gray-700 hover:border-gray-600" : "bg-gray-50 text-gray-600 border-gray-200 hover:border-gray-300"
                      }`}
                    >
                      {m}
                    </button>
