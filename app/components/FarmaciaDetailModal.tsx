...
export default function FarmaciaDetailModal({ isOpen, onClose, farmacia: initial, onFarmaciaUpdate, onStartDrag }: FarmaciaDetailModalProps) {
  ...
  return (
    ...
    >
      ...
      <div
        className="relative bg-white dark:bg-gray-900 rounded-t-2xl sm:rounded-2xl shadow-2xl w-full sm:max-w-md max-h-[85vh] overflow-y-auto"
        onClick={e => e.stopPropagation()}
      >
        {/* Header */}
        <div className={`sticky top-0 z-10 px-5 py-4 border-b ${farmacia.esDeturno ? "bg-green-50 dark:bg-green-900/30 border-green-200 dark:border-green-800" : "bg-white dark:bg-gray-900 border-gray-100 dark:border-gray-800"}`}>
          <div className="flex items-start justify-between gap-3">
            <div>
              {farmacia.esDeturno && (
                <span className="inline-block text-xs font-bold text-green-700 dark:text-green-400 bg-green-100 dark:bg-green-900/50 px-2 py-0.5 rounded-full mb-1">
                  🟢 DE TURNO HOY
                </span>
              )}
              <h2 className="text-lg font-bold text-gray-900 dark:text-white">Farmacia {farmacia.nombre}</h2>
              <p className="text-sm text-gray-500 dark:text-gray-400 mt-0.5">{farmacia.direccion}</p>
            </div>
            <button onClick={handleClose} className="p-1.5 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-800 flex-shrink-0">
        ...
        <div className="p-5 space-y-5">
          ...
          <div className="border border-amber-200 dark:border-amber-700 bg-amber-50 dark:bg-amber-900/20 rounded-xl p-4">
            {/* Botón drag */}
            {onStartDrag && (
              <button
                onClick={() => { onStartDrag(farmacia.id); handleClose(); }}
                className="w-full mb-3 flex items-center justify-center gap-2 py-2 bg-white dark:bg-gray-800 border border-amber-300 dark:border-amber-600 text-amber-700 dark:text-amber-400 text-sm font-semibold rounded-lg hover:bg-amber-50 dark:hover:bg-amber-900/30"
              >
            ...
            <div className="flex items-center gap-2 mb-3">
              ...
              <span className="text-sm font-semibold text-amber-800 dark:text-amber-300">Corregir ubicación del pin</span>
            </div>
            ...
            {relocateResult && (
              <div className="mt-3 p-3 bg-white dark:bg-gray-800 border border-green-300 dark:border-green-700 rounded-lg">
                <p className="text-xs text-green-700 dark:text-green-400 font-medium flex items-center gap-1 mb-2">
