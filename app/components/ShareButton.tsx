...
export default function ShareButton({ url, title, text, isDark = true, compact = false }: ShareButtonProps) {
  ...
  const menuBg = isDark ? "bg-gray-800 border-gray-700" : "bg-white border-gray-200 shadow-xl";

  return (
    <div className="relative">
      <button
        onClick={handleNativeShare}
        className={`flex items-center gap-1.5 px-3 py-2 rounded-xl text-sm font-medium transition-all ${btnBase} ${compact ? "px-2" : ""}`}
        title="Compartir"
      >
      ...
      {open && (
        <>
          ...
          <div className={`absolute right-0 bottom-full mb-2 z-50 rounded-2xl border p-2 min-w-[180px] ${menuBg}`}>
            <button
              onClick={shareWhatsApp}
              className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm transition-colors ${isDark ? "hover:bg-gray-700 text-gray-200" : "hover:bg-gray-50 text-gray-700"}`}
            >
