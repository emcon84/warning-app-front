...
function MessageTicks({ read, onDark }: { read: boolean; onDark: boolean }) {
  const color = read
    ? onDark ? "#a5b4fc" : "#22c55e"
    : onDark ? "#818cf8" : "#9ca3af";
...
export default function ChatClient({ conversationId }: Props) {
  ...
  function handleInputChange(value: string) {
    ...
    if (nowMs - lastTypingSentRef.current > 2000 && wsRef.current?.readyState === WebSocket.OPEN) {
      ...
      <div className={`fixed top-14 left-0 right-0 z-40 border-b px-4 py-3 ${headerBg}`}>
        <div className="max-w-xl mx-auto flex items-center gap-3">
          ...
          <div className={`w-9 h-9 rounded-full overflow-hidden flex-shrink-0 flex items-center justify-center font-bold text-sm ${avatarBg}`}>
            {pro.foto
              ? <Image src={pro.foto} alt="" width={36} height={36} className="w-full h-full object-cover" unoptimized />
