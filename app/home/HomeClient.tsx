...
export default function HomeClient({ professionals, comercios, turno, supermarkets }: Props) {
  ...
  useEffect(() => {
    const t = setInterval(() => setPromoBannerSlide(p => (p + 1) % 3), 7000);
    return () => clearInterval(t);
  }, []);
        ...
        <section className="px-0 mb-6">
          <div
            ...
            onPointerUp={(e) => {
              ...
              if (d > 50) setPromoBannerSlide(p => (p + 1) % 3);
              else if (d < -50) setPromoBannerSlide(p => (p - 1 + 3) % 3);
            }}
          >
            <AnimatePresence mode="wait">
              ...
              {promoBannerSlide === 0 && (
                <motion.div key="whatsapp" variants={slideVariants} initial="enter" animate="center" exit="exit" transition={{ duration: 0.45, ease: [0.22, 1, 0.36, 1] }} className="absolute inset-0">
                  <img src="https://images.unsplash.com/photo-1441986300917-64674bd600d8?w=800&h=400&fit=crop&q=80" alt="" className="absolute inset-0 w-full h-full object-cover object-center" />
                  <div className="absolute inset-0 bg-gradient-to-r from-emerald-950/98 via-emerald-950/88 to-emerald-900/55" />
                  {/* WA icon decorativo — visible en todos */}
