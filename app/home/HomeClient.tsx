...
export default function HomeClient({ professionals, comercios, turno, supermarkets }: Props) {
  ...
  const promoBannerSwipeX = useRef(0);
  const [promoBannerSlide, setPromoBannerSlide] = useState(0);
  ...
  useEffect(() => {
    const t = setInterval(() => setPromoBannerSlide(p => (p + 1) % 2), 7000);
    return () => clearInterval(t);
  }, []);
    ...
    <div className={`min-h-screen ${bg} ${textPrimary}`}>
      ...
      <div className="max-w-xl md:max-w-5xl mx-auto px-4 md:px-8 pt-6 pb-32">
        ...
        <section className="px-0 mb-6">
          <div
            className="relative overflow-hidden rounded-2xl h-44 md:h-52 touch-pan-y"
            onPointerDown={(e) => { promoBannerSwipeX.current = e.clientX; }}
            onPointerUp={(e) => {
              const d = promoBannerSwipeX.current - e.clientX;
              if (d > 50) setPromoBannerSlide(p => (p + 1) % 2);
              else if (d < -50) setPromoBannerSlide(p => (p - 1 + 2) % 2);
            }}
          >
            <AnimatePresence mode="wait">

              {/* ── Slide 1: WhatsApp Orders ── */}
              {promoBannerSlide === 0 && (
                <motion.div
                  key="whatsapp"
                ...
                >
                  ...
                  />
                  <div className="absolute inset-0 bg-gradient-to-r from-emerald-950/98 via-emerald-950/88 to-emerald-900/60" />
                  <div className="relative z-10 h-full flex flex-col justify-center px-5 gap-1.5 w-[80%] md:max-w-[55%]">
                    ...
                    <Link href="/comercios" className="inline-flex items-center gap-1.5 font-bold text-xs px-4 py-2 rounded-full w-fit mt-0.5 transition-opacity hover:opacity-90 text-white" style={{ background: "#25D366" }}>
              ...
              {promoBannerSlide === 1 && (
                <motion.div
                  ...
                  exit="exit"
                  transition={{ duration: 0.45, ease: [0.22, 1, 0.36, 1] }}
                  className="absolute inset-0 bg-gradient-to-br from-violet-950 via-purple-900 to-indigo-900"
                >
                  {/* Ambient glow */}
                  <div className="absolute top-0 right-0 w-48 h-48 rounded-full bg-purple-500/20 blur-3xl pointer-events-none" />
                  <div className="absolute bottom-0 left-1/3 w-36 h-36 rounded-full bg-violet-500/15 blur-2xl pointer-events-none" />

                  {/* Text — izquierda */}
                  <div className="relative z-10 h-full flex flex-col justify-center px-5 gap-1.5 w-[75%] md:max-w-[52%]">
                    <span className="text-[10px] font-semibold uppercase tracking-wider text-purple-300/70">RED SOCIAL LOCAL</span>
                    <h3 className="text-white font-bold text-base md:text-lg leading-tight">Seguí a tus comercios favoritos</h3>
                    <p className="text-white/70 text-xs leading-relaxed line-clamp-2">Sumarte a un comercio y recibí sus ofertas, sorteos y novedades en tiempo real.</p>
                    <Link href="/comercios" className="inline-flex items-center gap-1.5 font-bold text-xs px-4 py-2 rounded-full w-fit mt-0.5 bg-purple-500 hover:bg-purple-400 text-white transition-colors">
                      Explorar comercios <ChevronRight className="w-3 h-3" />
                    </Link>
                  </div>

                  {/* Phone mockup — solo desktop */}
                  <div className="hidden md:block absolute right-6 top-1/2 -translate-y-1/2 z-10">
                    <div className="relative" style={{ width: 128, height: 196 }}>
                      <div className="h-full rounded-[22px] border-2 border-white/15 bg-gray-950 overflow-hidden shadow-[0_20px_60px_rgba(0,0,0,0.65)]">
                        <div className="absolute top-0 left-1/2 -translate-x-1/2 w-7 h-2 bg-black rounded-b-xl z-20" />
                        <div className="absolute inset-0 pt-4 px-2 pb-2 flex flex-col gap-1 overflow-hidden">
                          {/* Header tienda */}
                          <div className="h-9 rounded-xl bg-gray-800 flex items-center justify-center">
                            <span className="text-[8px] text-white/70 font-bold tracking-wide">EL ESTRIBO</span>
                          </div>
                          {/* Botón WA */}
                          <div className="h-5 rounded-lg flex items-center justify-center gap-1" style={{ background: "#25D366" }}>
                            <span className="text-[6.5px] text-white font-bold">Contactar WhatsApp</span>
                          </div>
                          {/* Recomendaste */}
                          <div className="h-5 rounded-lg border border-green-700/60 bg-green-900/20 flex items-center justify-center">
                            <span className="text-[6.5px] text-green-400">✓ Recomendaste</span>
                          </div>
                          {/* Sumate — destacado */}
                          <div className="relative mt-auto">
                            <div className="rounded-xl border border-amber-500/60 bg-amber-500/10 px-2 py-1.5 flex items-center gap-1.5">
                              <div className="w-5 h-5 rounded-full bg-amber-500/25 flex items-center justify-center flex-shrink-0">
                                <svg className="w-3 h-3 text-amber-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6.002 6.002 0 00-4-5.659V5a2 2 0 10-4 0v.341C7.67 6.165 6 8.388 6 11v3.159c0 .538-.214 1.055-.595 1.436L4 17h5m6 0v1a3 3 0 11-6 0v-1m6 0H9" />
                                </svg>
                              </div>
                              <div className="flex-1 min-w-0">
                                <p className="text-[7px] text-white font-bold leading-none">Sumate</p>
                                <p className="text-[5.5px] text-white/50 leading-tight mt-0.5">Recibí novedades</p>
                              </div>
                            ...
                            {/* Tap indicator */}
                            <div className="absolute -bottom-5 right-3 text-[10px] select-none">👆</div>
                          </div>
                        </div>
                      </div>
                      {/* Reflejo superior */}
                      <div className="absolute top-[6px] left-[6px] right-[6px] h-14 bg-gradient-to-b from-white/5 to-transparent rounded-t-[18px] pointer-events-none" />
                    </div>
                  </div>
                </motion.div>
              )}
            ...
            <div className="absolute bottom-3 left-5 flex gap-1.5 z-20">
              {[0, 1].map(i => (
                <button
                  key={i}
                  onClick={() => setPromoBannerSlide(i)}
                  className={`rounded-full transition-all duration-300 ${i === promoBannerSlide ? "w-5 h-1.5 bg-white" : "w-1.5 h-1.5 bg-white/40 hover:bg-white/60"}`}
                />
              ))}
            </div>
          </div>
