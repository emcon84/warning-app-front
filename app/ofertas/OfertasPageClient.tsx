...
export default function OfertasPageClient({ initialSupermarkets }: Props) {
  ...
  const handleAddSupermarket = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setSupermarkets((prev) => [...prev, created].sort((a, b) => a.name.localeCompare(b.name)));
      setNewName("");
      setNewAddress("");
      setIsAddOpen(false);
    } catch (err) {
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-950">
            ...
            {supermarkets.map((supermarket) => (
              <div key={supermarket.id} className="relative group">
                <Link
                  ...
                  className="w-full bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-2xl p-4 shadow-sm flex flex-col items-center gap-3 hover:border-green-400 dark:hover:border-green-600 transition-colors active:scale-95 block"
                >
                ...
                <button
                  onClick={() => setConfirmDelete(supermarket)}
      ...
      {/* Modal agregar */}
      {isAddOpen && (
        <div className="fixed inset-0 flex items-end sm:items-center justify-center p-4" style={{ zIndex: 1200, backgroundColor: "rgba(0,0,0,0.6)", backdropFilter: "blur(4px)" }} onClick={() => setIsAddOpen(false)}>
          <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-2xl w-full max-w-md" onClick={(e) => e.stopPropagation()}>
            ...
            <form onSubmit={handleAddSupermarket} className="px-5 py-4 space-y-4">
              <div>
                ...
                <input type="text" value={newName} onChange={(e) => setNewName(e.target.value)} placeholder="Ej: Super Reconquista - Centro" className={inputClass} required autoFocus />
              </div>
              <div>
                <label className="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-1.5">Dirección</label>
                <input type="text" value={newAddress} onChange={(e) => setNewAddress(e.target.value)} placeholder="Ej: Mitre 480" className={inputClass} />
              </div>
              <div className="flex gap-3 pt-1">
                <button type="button" onClick={() => setIsAddOpen(false)} className="flex-1 px-4 py-2.5 bg-gray-200 dark:bg-gray-700 text-gray-800 dark:text-gray-200 rounded-xl font-semibold text-sm">Cancelar</button>
                <button type="submit" disabled={saving} className="flex-1 px-4 py-2.5 bg-green-500 hover:bg-green-600 disabled:opacity-50 text-white rounded-xl font-semibold text-sm">{saving ? "Guardando..." : "Guardar"}</button>
              </div>
            </form>
          </div>
        </div>
      ...
      {confirmDelete && (
        <div className="fixed inset-0 flex items-center justify-center p-4" style={{ zIndex: 1300, backgroundColor: "rgba(0,0,0,0.6)", backdropFilter: "blur(4px)" }} onClick={() => setConfirmDelete(null)}>
          <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-2xl w-full max-w-sm p-6" onClick={(e) => e.stopPropagation()}>
            ...
            <h3 className="text-base font-bold text-gray-900 dark:text-white text-center mb-1">Eliminar supermercado</h3>
            <p className="text-sm text-gray-500 dark:text-gray-400 text-center mb-6">
              ¿Eliminás <span className="font-semibold text-gray-900 dark:text-white">{confirmDelete.name}</span> y todas sus ofertas?
            </p>
            <div className="flex gap-3">
              <button onClick={() => setConfirmDelete(null)} className="flex-1 px-4 py-2.5 bg-gray-200 dark:bg-gray-700 text-gray-800 dark:text-gray-200 rounded-xl font-semibold text-sm">Cancelar</button>
              <button onClick={handleDeleteConfirm} className="flex-1 px-4 py-2.5 bg-red-500 hover:bg-red-600 text-white rounded-xl font-semibold text-sm">Eliminar</button>
            </div>
          </div>
        </div>
      )}
