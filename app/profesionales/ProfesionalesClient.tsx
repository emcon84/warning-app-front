...
function FeaturedCard({ pro, dark }: { pro: Professional; dark: boolean }) {
  const isTest = pro.slug.startsWith("test-");
  return (
    <Link href={`/profesional/${pro.slug}`} className="block">
      <div className="flex flex-col items-center gap-2 cursor-pointer group w-full">
        ...
        {/* Info — ancho fijo para truncar en lugar de wrappear */}
        <div className="text-center w-full overflow-hidden">
          <p className={`text-xs font-semibold truncate ${dark ? "text-white" : "text-gray-900"}`}>
            {pro.nombre}
          </p>
          {isTest && (
            <span className="inline-block text-[9px] font-bold px-1.5 py-0.5 rounded-full bg-amber-500/20 text-amber-400 border border-amber-500/40 leading-none mt-0.5">
              TEST
            </span>
          )}
