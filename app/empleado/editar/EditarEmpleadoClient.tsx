...
export default function EditarEmpleadoClient() {
  ...
  useEffect(() => {
    if (!isLoaded) return;
    if (!isSignedIn) { router.push("/profesionales"); return; }
    getToken().then(async (token) => {
      if (!token) return;
      try {
        const res = await fetch(`${API}/api/empleados/me`, { headers: { Authorization: `Bearer ${token}` } });
        if (!res.ok) { router.push("/empleado/nuevo"); return; }
  ...
  if (saved) {
    return (
      <div className={`min-h-screen ${bg} flex items-center justify-center flex-col gap-4`}>
        <CheckCircle className="w-12 h-12 text-green-400" />
        <p className={`text-lg font-semibold ${isDark ? "text-white" : "text-gray-900"}`}>Perfil actualizado</p>
      </div>
    );
  }
    ...
    <div className={`min-h-screen ${bg} pb-24 md:pb-0`}>
      ...
      <div className="max-w-lg mx-auto px-4 py-8">
        <div className="flex items-center gap-3 mb-6">
          ...
          <h1 className={`text-lg font-bold ${isDark ? "text-white" : "text-gray-900"}`}>Editar perfil</h1>
        </div>
        ...
        <div className={`rounded-2xl border p-6 flex flex-col gap-5 ${cardBg}`}>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="flex flex-col gap-1.5">
              ...
              <input type="text" value={nombre} onChange={(e) => setNombre(e.target.value)} className={`w-full px-4 py-3 rounded-xl border text-sm focus:outline-none transition-colors ${inputClass}`} />
            </div>
            <div className="flex flex-col gap-1.5">
              <label className={`text-sm font-medium ${labelClass}`}>Apellido *</label>
              <input type="text" value={apellido} onChange={(e) => setApellido(e.target.value)} className={`w-full px-4 py-3 rounded-xl border text-sm focus:outline-none transition-colors ${inputClass}`} />
            </div>
          </div>

          {/* Habilidades */}
          <div className="flex flex-col gap-1.5">
            <label className={`text-sm font-medium ${labelClass}`}>Habilidades *</label>
            ...
            <div className="flex flex-wrap gap-1.5">
              {HABILIDADES_SUGERIDAS.filter((h) => !habilidades.includes(h)).slice(0, 8).map((h) => (
                <button key={h} onClick={() => addHabilidad(h)} className={`px-2.5 py-1 rounded-full text-xs transition-colors ${isDark ? "bg-gray-800 text-gray-400 hover:text-white border border-gray-700" : "bg-gray-100 text-gray-600 hover:bg-gray-200 border border-gray-200"}`}>+ {h}</button>
              ))}
            </div>
          </div>
          ...
          <div className="flex flex-col gap-1.5">
            ...
            <textarea value={descripcion} onChange={(e) => setDescripcion(e.target.value)} rows={4} className={`w-full px-4 py-3 rounded-xl border text-sm focus:outline-none transition-colors resize-none ${inputClass}`} />
          </div>

          {/* Barrio / WhatsApp */}
          <div className="grid grid-cols-2 gap-3">
            <div className="flex flex-col gap-1.5">
              <label className={`text-sm font-medium ${labelClass}`}>Barrio</label>
              <select value={barrio} onChange={(e) => setBarrio(e.target.value)} className={`w-full px-4 py-3 rounded-xl border text-sm focus:outline-none transition-colors ${inputClass}`}>
                <option value="">Sin especificar</option>
                {BARRIOS.map((b) => <option key={b} value={b}>{b}</option>)}
              </select>
            </div>
            <div className="flex flex-col gap-1.5">
              <label className={`text-sm font-medium ${labelClass}`}>WhatsApp</label>
              <input type="tel" value={whatsapp} onChange={(e) => setWhatsapp(e.target.value)} placeholder="3482123456" className={`w-full px-4 py-3 rounded-xl border text-sm focus:outline-none transition-colors ${inputClass}`} />
            </div>
          </div>
          ...
          {error && <div className="px-4 py-3 rounded-xl bg-red-900/30 border border-red-700 text-red-400 text-sm">{error}</div>}

          <button
            onClick={handleSave}
            disabled={saving || !nombre.trim() || !apellido.trim() || habilidades.length === 0}
